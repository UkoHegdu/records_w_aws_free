#!/usr/bin/env node
/**
 * Copy handler and shared files from terraform/lambda/ to backend/src/lambda/.
 * Run from repo root after editing terraform/lambda/ so the backend stays in sync.
 * Usage: node backend/scripts/sync-lambda-from-terraform.js
 */
...
const SRC = path.join(REPO_ROOT, 'terraform/lambda');
const DST = path.join(REPO_ROOT, 'backend/src/lambda');
...
console.log('Done. Copied', copied, 'files to backend/src/lambda/');
